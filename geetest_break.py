...
def suiji_message():
    return Vd_my() + Vd_my() + Vd_my() + Vd_my()
...
def breaken():
    ...
    track_list1, gt, new_challenge, session = parse_res()
    if track_list1:
        print(track_list1)
        print("轨迹的距离：{}".format(track_list1[-1][0]), new_challenge)
        ...
        I1P = {
            "lang": "zh-cn",
            "userresponse": parse3_h8(ll=track_list1[-1][0], change=new_challenge),  # 距离l,change 用h8函数加密
            "passtime": track_list1[-1][-1],  # 轨迹的最后参数
            "imgload": random.randint(100, 200),  # 图片加载时间
            "aa": aa,  # 轨迹池来模拟得出 正确！
            "ep": ep,  # 上面给出是字典形式
